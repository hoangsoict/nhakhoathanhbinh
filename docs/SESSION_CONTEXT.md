...
Project là website đặt lịch cho phòng khám/nha khoa Thanh Bình. Khách hàng thao tác công khai, không cần tạo tài khoản, không cần đăng nhập, không dùng mã lịch hẹn. Khách hàng dùng số điện thoại để đặt lịch, tra cứu lịch, sửa lịch và hủy lịch. Khu vực Admin/Lễ tân dùng nội bộ để xem danh sách đặt lịch, cập nhật trạng thái, cấu hình lịch làm việc, ngày nghỉ nội bộ và thông tin hiển thị trên trang chủ.
...
- `app/page.tsx`: UI chính cho khách hàng và Admin.
...
- `app/page.tsx`: thêm flow Admin nhập PIN trước khi hiện tác vụ, tab con Admin, dropdown giờ 30 phút, config trang chủ, config ngày nghỉ.
...
- `app/api/admin/appointments/route.ts`: API Admin danh sách/cập nhật trạng thái, thông báo PIN rõ ràng.
...
- Admin chỉ thấy tác vụ sau khi nhập đúng `ADMIN_PIN`.
- Admin có tab `Danh sách đặt lịch`, `Cấu hình lịch làm việc`, `Thông tin trang chủ`.
...
- API đặt/sửa lịch chặn slot đã đủ 04 khách.
...
- Admin/Lễ tân hiện dùng PIN tĩnh, chưa có hệ thống login/session thật.
...
- Lỗi "Không có quyền truy cập" gây nhầm khi gọi API admin: đổi thông báo thành "Vui lòng nhập đúng PIN admin"; xác nhận endpoint sửa lịch khách `/api/appointments` không yêu cầu PIN.
...
- `clinic_settings` lưu cấu hình dạng JSONB: `weekly_schedule`, `internal_holidays`, `homepage_content`.
...
- `ADMIN_PIN`: PIN nội bộ mở trang Admin.

## Việc Tiếp Theo Sau Khi Mở Lại Session

1. Chạy lại `supabase/schema.sql` trên Supabase để cập nhật `internal_holidays` và `homepage_content`.
2. Kiểm tra Vercel đã có đủ env: `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `DATABASE_URL`, `ADMIN_PIN`.
